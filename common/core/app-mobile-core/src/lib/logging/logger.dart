...
class PSLogger {
  static const _appName = 'main_app';
  ...
  /// Send the API errors to the Analytics and print on the console.
  static void logAPIError<T>(String msg) async {
    final detailedMessage = _getDetailedMessage(msg, LoggerLevel.API_ERROR, T.toString());
    if (detailedMessage.isNotEmpty) {
      _logInAnalytics(
        'api_error',
        {'errorMsg': detailedMessage},
      );
      _logMessage(Level.info, detailedMessage);
    }
  }
    ...
    if (kReleaseMode) {
      if (_analytics == null) {
        try {
          ...
          // do nothing
          PSLogger.logDebug('could not find analytics object');
        }
      }
...
class PSConsoleOutput extends LogOutput {
  ...
  void output(OutputEvent event) {
    PSLogger.logs.addAll(event.lines);
