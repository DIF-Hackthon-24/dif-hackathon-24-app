...
extension TaskManagerDataRequestRestTaskHandler on TaskManager {
  Future<BaseDataModel?> handleRestNetworkRequest<T>(Task task) async {
  ...
  Future<BaseDataModel?> _kickStartRequest<T>(RestRequest request, Task task) async {
    // Make the network call
    final client = DIContainer.container<INetworkClient>(
      NetworkManager.networkClientKey,
    );
    final value = await client.runRestRequest(request: request);
    if (value.error != null) {
      throw value.error!;
    }
