...
import 'package:task_manager/task_manager_impl/data/task_manager_data_file_download_handler_extension.dart';
...
class TaskManager {
  ...
  Future waitForExecute<T>(Task task) async {
    switch (task.taskType) {
      ...
      case TaskType.DATA_OPERATION:
        final result = _handleDataRequest<T>(task).catchError(
          (error) {
            throw error;
          },
        );
  ...
  Future<dynamic> _handleDataRequest<T>(Task task) async {
    switch (task.subType) {
      ...
      case TaskSubType.REST_FILE_UPLOAD:
        final result = await handleRestFileUploadTask(task).catchError((error){
          throw error;
        });
      return result;
      case TaskSubType.FILE_DOWNLOAD:
        return await handleFileDownloadTask(task);
